...
import IsoSchema from "./venezuela_schema.json"; // Importa el esquema JSON para validar los datos.
        ...
        // Recorre cada objeto "iso" en el arreglo "isos".
            ...
            // Si la validación es exitosa
                isosValidos.push(iso);
                // Muestra el objeto "iso" válido.
            }
        });

    // 5. Función 1: Usar reduce para contar el total de municipios
    const totalMunicipios = isosValidos.reduce((acumulador, iso) => acumulador + iso.municipios.length, 0);
    ...
    const nombresEstadosCapitales = isosValidos.map(iso => ({
